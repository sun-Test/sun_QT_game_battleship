...
bool hmi::parsePosition(const string &strPosition)
{
...
void hmi::handleHitFire(bool bGetHit)
{
    if(bGetHit)
    {
        cout<<"you have hit one!!"<<endl;
    }
    else
    {
...
void hmi::handleMultipleFired()
{
    cout<<"you have already fired at this place..."<<endl;
}
void hmi::handleFleetSunk(int iScore_in)
{
     cout<<"Congratulation, you have sunk all ships!!!\n  Your score is "<< iScore_in<<endl<<"reset the game input 's'"<<endl;
}
void hmi::handleSunkFire(const ship_2d &cref_sunkShip)
{
    cout<<"you have sunk one ship!!" <<endl;
    emit requestGameRoundState();
}
void hmi::handleGameRoundeFinished()
{
    cout<<"this game round has been finished!!!\n"<<"reset the game input 's'"<<endl;
}
void hmi::handleReportBoardState(unordered_set<int> firedPositions, unordered_set<int> hitPositions, unordered_set<int> sunkPositions)
{
